...
namespace Utp
{
	...
	public class UtpTransport : Transport
	{
		...
		/// <summary>
		/// The Relay server data if using UtpTransport in conjunction with Unity Multiplayer Services sessions.
		/// </summary>
		...
		private void Awake()
		{
			...
			server = new UtpServer(
                ...
                (connectionId) =>
                {
                    string address = server.GetClientAddress(connectionId);
                    OnServerConnectedWithAddress?.Invoke(connectionId, address);
                },
		...
		private void SetupDefaultCallbacks()
		{
			...
			/*
            OnServerConnectedWithAddress ??= (connId, clientAddress) => UtpLog.Warning("OnServerConnectedWithAddress called with no handler");
			...
			*/
			OnServerConnectedWithAddress ??= (connId, clientAddress) => { };
		...
		public override void ClientConnect(string address)
		{
			...
			if (useRelay)
			{
				...
				{
                    client.RelayConnect(relayServerData);
                }
				else
				{
        ...
        /// <summary>
        /// Returns the server's or client's local endpoint - whichever is running at the moment.
        /// </summary>
		/// <exception cref="InvalidOperationException">Thrown when both server and client are inactive.</exception>
		/// <returns>The local endpoint if available.</returns>
        public NetworkEndpoint GetLocalEndpoint()
        {
			...
			{
				throw new InvalidOperationException("Unable to get the local endpoint: both server and client are inactive.");
			}
        }
        ...
        /// <param name="relayServerData">RelayServerData object obtained when starting a Unity Multiplayer Services session.</param>
		...
		{
			if (useRelay && !relayServerData.Equals(default(RelayServerData)))
			{
                server.Start(relayServerData);
			}
			else
			{
                // The data we need to connect is embedded in the relayManager's ServerAllocation
