...
namespace Utp
{
	...
	public class UtpServer : UtpEntity
	{
        ...
        /// <summary>
        /// Initialize the server using Relay and already existing RelayServerData object. Currently only supports IPV4.
        /// </summary>
        /// <param name="relayServerData">The Relay server data.</param>
        public void Start(RelayServerData relayServerData)
        {
            if (IsNetworkDriverInitialized())
            {
                UtpLog.Warning("Attempting to start a server that is already active.");
            ...
            //Instantiate relay network settings
            RelayNetworkParameter relayNetworkParameter = new RelayNetworkParameter { ServerData = relayServerData };
            NetworkSettings networkSettings = new NetworkSettings();
            ...
            UtpLog.Info("Relay server started");
        }
